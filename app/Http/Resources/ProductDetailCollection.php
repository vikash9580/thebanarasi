...
class ProductDetailCollection extends ResourceCollection
{
        ...
        return [
            'data' => $this->collection->map(function($data) {
                return [
                    ...
                    'varient'=>$this->convertToVarient($data->id),
                    'links' => [
    ...
    protected function convertToVarient($data)
    {
        $list=ProductStock::where('product_id',$data)->get();
        $total=count($list);
        for($i=0;$i<$total;$i++)
        {   
            if($list[$i]['discounts_type']=='percent')
            {
            $list[$i]['discount_price']=$list[$i]['price']-($list[$i]['price']*$list[$i]['discounts']/100);
            }
            else
            {
                $list[$i]['discount_price']=$list[$i]['price']-$list[$i]['discounts'];
            }
            $list[$i]['photos']=[];
        } 
        return $list;
