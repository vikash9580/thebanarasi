...
class HomeController extends Controller
{
    ...
    public function wholesale_customer_registration(Request $request)
    {
        if(Auth::check()){
            return redirect()->route('home');
        }
     ...
     public function request_sample(Request $request)
    {
        ...
        return view('frontend.request_sample');
    }
    
      public function request_sample_payment(Request $request)
    {
        
          $request->session()->put('payment_type', 'sample_payment');
           $request->session()->put('name', $request->name);
            $request->session()->put('contact', $request->phone);
    ...
    public function admin_dashboard()
    {
        ...
        //return $qwer = Order::get()->with('user');
       ...
       /* $dates = collect();
        ...
        $posts = Order::select('orders.*','users.id as u_id', 'users.user_type as u_type')->leftjoin('users', 'orders.user_id', 'users.id')->where('users.user_type', 'reseller')->where( 'orders.created_at', '>=', $dates->keys()->first() )
                     ->groupBy( 'date' )
                     ->orderBy( 'date' )
                     ->get( [
        ...
        return $users = $dates->merge( $posts );*/
        ...
        $Order = Order::where( 'created_at', '>=', $dates->keys()->first() )
                      ->orderBy( 'date' )  
                     ->get( [
                         DB::raw( 'DATE( created_at ) as date' ),
                         DB::raw( 'COUNT( * ) as "count"' )
        ...
        $order_in_mounts=DB::table('orders')->select(DB::raw('count(id) as `data`'), DB::raw("DATE_FORMAT(created_at, '%M-%Y') new_date"),  DB::raw('YEAR(created_at) year, MONTH(created_at) month'))->groupby('year','month')->take(6)->get();
        
        $pending_order_in_mounts=DB::table('order_details')->where('delivery_status', 'pending')->select(DB::raw('count(id) as `data`'), DB::raw("DATE_FORMAT(created_at, '%M-%Y') new_date"),  DB::raw('YEAR(created_at) year, MONTH(created_at) month'))->groupby('year','month')->get();
        
        $confirm_order_in_mounts=DB::table('order_details')->where('delivery_status', 'confirmed')->select(DB::raw('count(id) as `data`'), DB::raw("DATE_FORMAT(created_at, '%M-%Y') new_date"),  DB::raw('YEAR(created_at) year, MONTH(created_at) month'))->groupby('year','month')->get();
        ...
        $cod_payment = Order::where('payment_type', 'cash_on_delivery')->where( 'created_at', '>=', $dates->keys()->first() )
                      ->orderBy( 'date' )  
                     ->get( [
                         DB::raw( 'DATE( created_at ) as date' ),
                         DB::raw( 'COUNT( * ) as "count"' )
        ...
        $online_payment = Order::where('payment_type', '!=', 'cash_on_delivery')->where( 'created_at', '>=', $dates->keys()->first() )
                      ->orderBy( 'date' )  
                     ->get( [
                         DB::raw( 'DATE( created_at ) as date' ),
                         DB::raw( 'COUNT( * ) as "count"' )
        ...
        $cod_payment_mounts=DB::table('orders')->where('payment_type', 'cash_on_delivery')->select(DB::raw('count(id) as `data`'), DB::raw("DATE_FORMAT(created_at, '%M-%Y') new_date"),  DB::raw('YEAR(created_at) year, MONTH(created_at) month'))->groupby('year','month')->get();
        
        $online_payment_mounts=DB::table('orders')->where('payment_type', '!=', 'cash_on_delivery')->select(DB::raw('count(id) as `data`'), DB::raw("DATE_FORMAT(created_at, '%M-%Y') new_date"),  DB::raw('YEAR(created_at) year, MONTH(created_at) month'))->groupby('year','month')->get();
        
        return view('backend.dashboard', compact('orders', 'order_in_mounts', 'pending_order_in_mounts', 'confirm_order_in_mounts', 'cod_payment', 'online_payment', 'cod_payment_mounts', 'online_payment_mounts'));
        // return view('test');
    }
    ...
    public function trackOrder(Request $request)
    {
        if($request->has('order_code')){
            //  $order = Order::where('code', $request->order_code)->first();
              $order = Order::where('code', $request->order_code)->first();
            if($order != null){
               flash(translate('Order Track Successfully'))->success();
    ...
    public function search(Request $request)
    {
        ...
        $non_paginate_products = filter_products($products)->orderBy('id','desc')->get();
    ...
    public function variant_price(Request $request)
    {
        ...
        if($str != null && $product->variant_product){
            ...
            $thumbnail_img=$product_stock->thumbnail_img;
        }
        else{
        ...
        return array('price' => single_price($price*$request->quantity), 'quantity' => $quantity, 'digital' => $product->digital,'thumbnail_img'=>uploaded_asset($thumbnail_img));
    }
    ...
    public function sellerpolicy(){
     ...
     public function whatsapp(){
      ...
      public function view_whatsapp(Request $request){
          
        $sort_search =null;
        $whatsapps = Whatsapp::orderBy('created_at', 'desc');
        if ($request->has('search')){
            $sort_search = $request->search;
            $whatsapps = $whatsapps->where('name', 'like', '%'.$sort_search.'%');
        }
        $whatsapps = $whatsapps->paginate(15);
        return view('backend.setup_configurations.whatsapp', compact('whatsapps', 'sort_search'));
    }
    ...
    public function number_store(Request $request){
        ...
        $whatsapp->save();
        return redirect()->route('whatsapp.view_whatsapp');
    }
     ...
     public function number_edit(Request $request,$id){
        ...
        $whatsapps = Whatsapp::orderBy('created_at', 'desc');
        $whatsapps = $whatsapps->paginate(15);
        $edit_data  = Whatsapp::findOrFail($id);
        return view('backend.setup_configurations.whatsapp', compact('edit_data','whatsapps','sort_search'));
    }
    
     public function number_update(Request $request){
      
          $whatsapp = Whatsapp::findOrFail($request->id);
          $whatsapp->name = $request->name;
          $whatsapp->number = $request->number;
          $whatsapp->save();
          return redirect()->route('whatsapp.view_whatsapp');
     ...
     public function number_destroy($id){
         $whatsapp = Whatsapp::findOrFail($id);
         $whatsapp->delete();
         flash(translate('Whatsapp has been deleted successfully'))->success();
         return redirect()->route('whatsapp.view_whatsapp');
    }
    ...
    {  
       	...
       	$data2 = Followup::where('next_date', '=', date('Y-m-d'))->where('next_time', '=', date("H:i"))->where('follow_up_status', 1)->where('delete_status', 1)->orderBy('id', 'DESC')->get();
       
       	if(count($data2)>0){
       	    	
       	foreach($data2 as $data){
       	    $user = User::where('id', $data->reseller_id)->first();
       	    $data->viewed=1;
       	    $data->save();
       	    sendWebNotification('Followup Notification',$user->name.' : '.$user->user_type); 
       	}
       	$notification_sound=\App\NotificationSound::where('id','>',0)->first();
       	$notification_sound->play=1;
       	$notification_sound->save();
       return 'Notification Send';
       	}
    }
    
    function notification_check(Request $request){
        
         return view('backend.inc.admin_notification')->render();
    }
