...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        if ($request->payment_option != null) {
            ...
            if ($request->payment_option != null) {
                if ($request->payment_option == 'paypal') {
                ...
                }elseif ($request->payment_option == 'ccavenue') {
                    $ccavenue = new CcavenueController;
                    return $ccavenue->ccavenue($request);
                    ...
                    return $paytm->index();
                     ...
                     $request->payment_option='cash_on_delivery';
                     $request->order_id=date('Ymd-His').rand(10,99);
                     ...
                     foreach(Cart::where('user_id',Auth::user()->id)->where('payment_status',0)->get() as $cart_list){
                      ...
                      $cart_data->order_id=  $request->order_id;
                      $cart_data->save();
    ...
    public function checkout_done($order_id, $payment)
    {
       ...
       // Session::put('cart', Session::get('cart')->where('owner_id', '!=', Session::get('owner_id')));
        ...
        return view('frontend.order_confirmed_online', compact('order'));
    }

    public function get_shipping_info(Request $request)
    {
        if (Cart::where('user_id',Auth::user()->id)->where('payment_status',0)->get()->count() > 0) {
        ...
        if (Session::has('coupon_discount')) {
            ...
            return view('frontend.shipping_info', compact('categories','total','address_id'));
        }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if (Cart::where('user_id',Auth::user()->id)->where('payment_status',0)->get() && Cart::where('user_id',Auth::user()->id)->where('payment_status',0)->get()->count() > 0) {
            $cart =Cart::where('user_id',Auth::user()->id)->where('payment_status',0)->get();
          ...
          //  $request->session()->put('cart', $cart);
            ...
            return view('frontend.payment_select', compact('total','cart'));
        } else {
    ...
    public function remove_coupon_code(Request $request)
    {
     ...
     public function apply_wallet(Request $request)
    {
        
         $request->session()->put('wallet_discount', Auth::user()->balance);
         return 1;
    }

   public function remove_wallet_discount(Request $request)
    {
