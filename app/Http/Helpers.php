...
if (! function_exists('flowinfo')) {
    function flowinfo($id)
    {
            $curl = curl_init();
            
            curl_setopt_array($curl, [
              CURLOPT_URL => "https://api.msg91.com/api/v5/flows/".$id,
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => "",
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 30,
              ...
              CURLOPT_HTTPHEADER => [
                "authkey: 222751Ao7RU0Wz5fbc9731P1"
              ],
            ]);
...
if (! function_exists('all_flows')) {
    function all_flows()
    {
            $curl = curl_init();
            
            curl_setopt_array($curl, [
              CURLOPT_URL => "https://api.msg91.com/api/v5/flows",
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => "",
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 30,
...
if (! function_exists('get_cached_sub_category_products')) {
    function get_cached_sub_category_products($subcategory_id = null) {
        $products = \App\Product::where('published', 1);
        $verified_sellers = verified_sellers_id();
        if(BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1){
            $products =  $products->where(function($p) use ($verified_sellers){
        ...
        if ($subcategory_id != null) {
            return Cache::remember('products-subcategory-'.$subcategory_id, 86400, function () use ($subcategory_id, $products) {
                return $products->where('subcategory_id', $subcategory_id)->latest()->take(12)->get();
            });
        }
        else {
            return Cache::remember('products', 86400, function () use ($products) {
...
if (! function_exists('home_prices')) {
    function home_prices($id)
    {
...
if (! function_exists('home_price_wholesales')) {
    function home_price_wholesales($id)
    {
        $product = Product::findOrFail($id);
...
if (! function_exists('home_price')) {
    function home_price($id)
    {
        ...
        $price=$product_stock[0];
        return format_price($price->price);
...
if (! function_exists('home_price_wholesale')) {
    function home_price_wholesale($id)
    {
        $product = Product::findOrFail($id);
         $product_stock=$product->stocks;
        $price=$product_stock[0];
            return format_price($price->wholesale_price);
...
if (! function_exists('home_discounted_price')) {
    function home_discounted_price($id)
    {
        ...
        if (!$inFlashDeal) {
            if($product->discounts_type == 'percent'){
                $price -= ($price*$product->discounts)/100;
            }
            elseif($product->discounts_type == 'amount'){
...
if (! function_exists('home_variant_discounted_price')) {
    function home_variant_discounted_price($id)
    {
        $products = ProductStock::findOrFail($id);
        $price=$products->price;
        
            if($products->discounts_type == 'percent'){
                $price -= ($price*$products->discounts)/100;
            }
            elseif($products->discounts_type == 'amount'){
                $price -= $products->discounts;
             }
           return $price;
...
if (!function_exists('errorMsg')) {
     function errorMsg($valid){
        
         $response = array(); 
       foreach ($valid->messages()->toArray() as $key => $value) { 
            
            $response[$key]=$value[0]; 
          }
  
        	return response()->json(['error'=>$response, 'status' =>'401'],401);
    }
}

if (!function_exists('errorMes')) {
     function errorMes($valid){
       ...
       foreach ($valid->messages()->toArray() as $key => $value) { 
          ...
          return response()->json(['message' =>$response ,'status'=>'401'], 401); 
    }
}
?>
