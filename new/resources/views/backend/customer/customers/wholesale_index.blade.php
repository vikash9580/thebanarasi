...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="align-items-center">
			<h1 class="h3">{{translate('All Wholesale Customers')}}</h1>
	</div>
</div>
        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th>{{translate('Validate Customer')}}</th>
                    ...
                    @if ($customer->user != null)
                        <tr>
							...
							<label class="aiz-switch aiz-switch-success mb-0">
                              <input onchange="update_published(this)" value="{{ $customer->user->id }}" type="checkbox" <?php if($customer->user->validate_wholesale_customer == 1) echo "checked";?> >
    ...
    <script type="text/javascript">
        ...
        function confirm_unban(url)
        {
         ...
         function update_published(el){
            ...
            $.post('{{ route('wholesle_customers.validate') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Customer Updated successfully') }}');
                }
                else{
