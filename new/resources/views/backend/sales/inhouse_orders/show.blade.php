    		...
    		<div class="clearfix float-right">
    			<table class="table">
          			<tbody>
                        ...
                        <tr>
                            <td><strong class="text-muted">{{translate('IGST')}} :</strong></td>
                            <td>{{ single_price($order->orderDetails->where('seller_id', $admin_user_id)->sum('tax')) }}</td>
                        </tr>
                        @endif
                        @if(json_decode($order->shipping_address)->state=='Uttar Pradesh')
            			<tr>
            				<td><strong class="text-muted">{{translate('CGST')}} :</strong></td>
            				<td>{{ single_price($order->orderDetails->where('seller_id', $admin_user_id)->sum('tax')/2) }}</td>
            			</tr>
                        <tr>
                            <td><strong class="text-muted">{{translate('SGST')}} :</strong></td>
                            <td>{{ single_price($order->orderDetails->where('seller_id', $admin_user_id)->sum('tax')/2) }}</td>
                        </tr>
                        @endif
                        <tr>
            			...
            			<tr>
            				...
            				<td class="text-muted h5">
                                ...
                                {{ single_price(($order->orderDetails->where('seller_id', $admin_user_id)->sum('price') + $order->orderDetails->where('seller_id', $admin_user_id)->sum('shipping_cost'))-$order->coupon_discount ) }}
            				</td>
            			</tr>
            			<tr>
            				<td><strong class="text-muted">{{translate('TOTAL')}}(Round-Off) :</strong></td>
            				<td class="text-muted h5">
            					<!-- {{ single_price($order->orderDetails->where('seller_id', $admin_user_id)->sum('price') + $order->orderDetails->where('seller_id', $admin_user_id)->sum('tax') + $order->orderDetails->where('seller_id', $admin_user_id)->sum('shipping_cost')) }} -->
                                Rs{{ round(($order->orderDetails->where('seller_id', $admin_user_id)->sum('price') + $order->orderDetails->where('seller_id', $admin_user_id)->sum('shipping_cost'))-$order->coupon_discount) }}.00
            				</td>
            			</tr>
          			</tbody>
    			</table>
