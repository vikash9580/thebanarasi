											...
											<div class="row gutters-5">
												<div class="col">
												 ...
												 <optgroup label="{{ $category->name }}">
												  <option value="cat_{{$category->id}}" @if( $value == 'cat_'.$category->id) selected @endif >{{ $category->name }} (Category)</option>
												 	@foreach (\App\SubCategory::where("category_id",$category->id)->get() as $key => $subcategory)
                                                      <option value="sub_{{$subcategory->id}}" @if( $value == 'sub_'.$subcategory->id) selected @endif >{{$subcategory->name}} (Sub Category)</option>
                                                   	@endforeach
                                                 </optgroup>
												@endforeach
							                            </select>
												...
												<select class="form-control aiz-selectpicker" name="home_categories[]" data-live-search="true" required>
												 ...
												 <optgroup label="{{ $category->name }}">
												  <option value="cat_{{$category->id}}">{{ $category->name }} (Category)</option>
												 	@foreach (\App\SubCategory::where("category_id",$category->id)->get() as $key => $subcategory)
                                                      <option value="sub_{{$subcategory->id}}">{{$subcategory->name}} (Sub Category)</option>
                                                   	@endforeach
                                                 </optgroup>
												@endforeach
												</select>
