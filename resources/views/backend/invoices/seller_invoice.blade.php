			...
			<table>
				...
				@else
					<tr>
						...
						<td style="font-size: 1.2rem;text-align:right;" class="strong">GSTIN:{{ get_setting('gstin') }}</td>
						<td class="text-right"></td>
					</tr>
					<tr>
	        ...
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        <tr>
			         ...
			         @if($shipping_address->state !='Uttar Pradesh')
			        <tr class="border-bottom">
			            <th class="gry-color text-left">{{ translate('CGST') }}</th>
			            <td class="currency">{{ single_price($order->orderDetails->where('seller_id', $user_id)->sum('tax')/2) }}</td>
			        </tr>
			        @endif
			        @if($shipping_address->state =='Uttar Pradesh')
			        <tr class="border-bottom">
			        ...
			        <tr>
			            ...
			            <td class="currency">{{ single_price($order->orderDetails->where('seller_id', $user_id)->sum('price') + $order->orderDetails->where('seller_id', $user_id)->sum('shipping_cost') - $order->coupon_discount ) }}</td>
			        </tr>
			        <tr>
			            <th class="text-left strong">{{ translate('Grand Total') }}(Round-off)</th>
			            <td class="currency">Rs{{ round($order->orderDetails->where('seller_id', $user_id)->sum('price') + $order->orderDetails->where('seller_id', $user_id)->sum('shipping_cost') - $order->coupon_discount ) }}.00</td>
			        </tr>
		        </tbody>
		    </table>
	    </div>
